...
>
>    ![image-20240806015913088](/posts/【TI】-1-Sysconfig工具使用（GPIO、定时器为例）/image-20240806015913088.png)
>
>    - gcc：代表C++方式编程的工程格式
...
![image-20240806020630210](/posts/【TI】-1-Sysconfig工具使用（GPIO、定时器为例）/image-20240806020630210.png)

# 二、GPIO口的基本配置

![image-20240806021554207](/posts/【TI】-1-Sysconfig工具使用（GPIO、定时器为例）/image-20240806021554207.png)

## 1. GPIO输出模式

简单配置Direction(方向)为Output即可

![image-20240806021652604](/posts/【TI】-1-Sysconfig工具使用（GPIO、定时器为例）/image-20240806021652604.png)
...
![image-20240811011437475](/posts/C:\Users\123\AppData\Roaming\Typora\typora-user-images\image-20240811011437475.png)

### 2.3. 外部中断

外部中断一般用于特定的事件检测，计数（如：里程），配合定时器测电机速度等，图示为**双边沿触发的外部中断**，并且中断优先级配置为1

![image-20240811012509997](/posts/【TI】-1-Sysconfig工具使用（GPIO、定时器为例）/image-20240811012509997.png)
...
![image-20240715192616803](/posts/【TI】-1-Sysconfig工具使用（GPIO、定时器为例）/image-20240715192616803.png)
...
![image-20240811012753418](/posts/【TI】-1-Sysconfig工具使用（GPIO、定时器为例）/image-20240811012753418.png)
...
![image-20240715193539699](/posts/【TI】-1-Sysconfig工具使用（GPIO、定时器为例）/image-20240715193539699.png)

### 5.1 **中断事件使能：**

与一般单片机不同，G3507配置中断后，程序中需要加上对中断事件类型的判断。

![image-20240715194038332](/posts/【TI】-1-Sysconfig工具使用（GPIO、定时器为例）/image-20240715194038332.png)
...
![image-20240715193738345](/posts/【TI】-1-Sysconfig工具使用（GPIO、定时器为例）/image-20240715193738345.png)

# #M0系列的低功耗工艺

![image-20240715194311900](/posts/【TI】-1-Sysconfig工具使用（GPIO、定时器为例）/image-20240715194311900.png)
...
![image-20240715214605017](/posts/【TI】-1-Sysconfig工具使用（GPIO、定时器为例）/image-20240715214605017.png)
...
![image-20240715215253775](/posts/【TI】-1-Sysconfig工具使用（GPIO、定时器为例）/image-20240715215253775.png)
...
![image-20240715215418979](/posts/【TI】-1-Sysconfig工具使用（GPIO、定时器为例）/image-20240715215418979.png)
...
![image-20240715215919260](/posts/【TI】-1-Sysconfig工具使用（GPIO、定时器为例）/image-20240715215919260.png)
...
![image-20240916125331399](/posts/【TI】-1-Sysconfig工具使用（GPIO、定时器为例）/image-20240916125331399.png)

可以看到，重新import的工程出现了CCS Theia识别图标

![image-20240916125517025](/posts/【TI】-1-Sysconfig工具使用（GPIO、定时器为例）/image-20240916125517025.png)
