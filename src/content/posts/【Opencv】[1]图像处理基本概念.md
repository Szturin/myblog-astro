...
# [](#一-图像处理常见基本概念)一、图像处理常见基本概念

## [](#1bgr和rgb格式)1.BGR和RGB格式
...
```python
import numpy as npimport cv2 as cv# 以灰度格式加载图像img_file = cv.imread("123.jpg",0)
```

### [](#2-显示图像与用户交互)2\. 显示图像与用户交互
...
```python
cv.imshow("img",img_file)cv.waitKey(0)cv.destroyAllWindows()
```
...
```python
import numpy as npimport cv2 as cvimg_file = cv.imread('img.jpg',0)cv.imshow('img',img_file)key = cv.waitKey(0)if key == 27 : # 按Esc退出    cv.destroyAllWindows()elif key == ord('s'): # 按s保存图像并退出    cv.imwrite('img_file_save.png',img_file)    cv.destroyAllWindows()
```
...
```python
# 使用matplotlib库import numpy as npimport cv2 as cvfrom matplotlib import pyplot as pltimg =  cv.imread('img.png',0)plt.imshow(img, cmap = 'gray', interpolation = 'bicubic')plt.xticks([]), plt.yticks([])  # 隐藏 x 轴和 y 轴上的刻度值plt.show()
```

## [](#二-视频入门)二、视频入门

### [](#1-驱动相机读取视频)1\. 驱动相机读取视频
...
```python
import cv2 as cvimport numpy as npfrom matplotlib import pyplot as pltcap = cv.VideoCapture(0)if not cap.isOpened():    print("can't not open camera")    exit()while True:    ret,frame = cap.read()        if not ret:        print('can not recevie frame')        break        gray = cv.cvtColor(frame, cv.COLOR_BGR2GRAY)    cv.imshow('video',gray)    if cv.waitKey(1) ==  ord('q'):        break        cap.release() # 释放视频捕获cv.destroyAllWindows()
```
...
```python
import cv2 as cvimport numpy as npfrom matplotlib import pyplot as pltcap = cv.VideoCapture(0)codec = cv.VideoWriter_fourcc('M', 'J', 'P', 'G')cap.set(cv.CAP_PROP_FOURCC, codec)cap.set(cv.CAP_PROP_FPS, 30) #帧数cap.set(cv.CAP_PROP_FRAME_WIDTH, 1920) #列 宽度cap.set(cv.CAP_PROP_FRAME_HEIGHT, 1080) # 行 高度if not cap.isOpened():    print("can't not open camera")    exit()while True:    ret,frame = cap.read()        if not ret:        print('can not recevie frame')        break        gray = cv.cvtColor(frame, cv.COLOR_BGR2GRAY)    cv.imshow('video',gray)    if cv.waitKey(1) ==  ord('q'):        break        cap.release() # 释放视频捕获cv.destroyAllWindows()
```

## [](#4-保存视频)4\. 保存视频

```python
import cv2 as cvimport numpy as npfrom matplotlib import pyplot as pltcap = cv.VideoCapture(0)fourcc = cv.VideoWriter_fourcc(*'XVID')out = cv.VideoWriter('output.avi',fourcc,20.0,(1920,1080))cap.set(cv.CAP_PROP_FPS, 30) #帧数cap.set(cv.CAP_PROP_FRAME_WIDTH, 1920) #列 宽度cap.set(cv.CAP_PROP_FRAME_HEIGHT, 1080) # 行 高度if not cap.isOpened():    print("can't open camera")    exit()while True:    ret,frame = cap.read()    if not ret:        print('not get')        frame = cv.flip(frame, 0)    out.write(frame)    cv.imshow("video",frame)    if cv.waitKey(1) == ord('q'):        break;cap.release()out.release()cv.destroyAllWindows()
```
...
```python
import numpy as npimport cv2 as cvdef nothing(x):    pass# 创建一个黑色的图像，一个窗口img = np.zeros((300,512,3), np.uint8)cv.namedWindow('image')# 创建RGB调整滑块栏cv.createTrackbar('R','image',0,255,nothing)cv.createTrackbar('G','image',0,255,nothing)cv.createTrackbar('B','image',0,255,nothing)# 为 ON/OFF 功能创建开关switch = '0 : OFF \n1 : ON'cv.createTrackbar(switch, 'image',0,1,nothing)while(1):    cv.imshow('image',img)    r = cv.getTrackbarPos('R','image')    g = cv.getTrackbarPos('G','image')    b = cv.getTrackbarPos('B','image')    s = cv.getTrackbarPos(switch,'image')    if s == 0:        img[:] = 0    else:        img[:] = [b,g,r]    if cv.waitKey(1) == ord('q'):        breakcv.destroyAllWindows()
```

# [](#四-opencv核心操作)四、OpenCV核心操作
...
```python
e1 = cv.getTickCount()# 你的执行代码e2 = cv.getTickCount()time = (e2 - e1)/ cv.getTickFrequency()
```

除了OpenCV，Python还提供了一个模块**time**，这有助于衡量执行时间。另一个模块**profile**有助于获取有关代码的详细报告，例如代码中每个函数花费了多少时间，调用了函数的次数等。

### [](#2-opencv默认的性能优化)2\. OpenCV默认的性能优化
...
```python
# 检查是否启用了优化In [5]: cv.useOptimized()Out[5]: TrueIn [6]: %timeit res = cv.medianBlur(img,49)10 loops, best of 3: 34.9 ms per loop# 关闭它In [7]: cv.setUseOptimized(False)In [8]: cv.useOptimized()Out[8]: FalseIn [9]: %timeit res = cv.medianBlur(img,49)10 loops, best of 3: 64.1 ms per loop
```
